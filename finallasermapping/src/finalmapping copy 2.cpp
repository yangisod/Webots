...
public:
    octreeGrid()
    {
        ...
        laserSub = n.subscribe<sensor_msgs::PointCloud2>("/Velodyne_PointCloud_Register", 1, &octreeGrid::laserCloudHandle2, this);
    }
    void laserCloudHandle(const sensor_msgs::PointCloud2ConstPtr &msg)
    {
        // pclCloudXYZ temp;
        ...
        // Publish(pubOctree_result, laserCloudReResult);
    ...
    void laserCloudHandle2(const sensor_msgs::PointCloud2ConstPtr &msg)
    {
        ...
        if (switchBuffersFlag < 5)
        {
            StoreFrame[switchBuffersFlag].octree.setInputCloud(laserCloudIn.makeShared());
            StoreFrame[switchBuffersFlag].octree.addPointsFromInputCloud();
            StoreFrame[switchBuffersFlag].octree.switchBuffers();
            StoreFrame[switchBuffersFlag].octreePre.setInputCloud(laserCloudIn.makeShared());
            StoreFrame[switchBuffersFlag].octreePre.addPointsFromInputCloud();

            StoreFrame[switchBuffersFlag].kdtree.setInputCloud(laserCloudIn.makeShared());
            switchBuffersFlag++;
            return;
        }

        switchBuffersIndex = switchBuffersIndex % 5;
        StoreFrame[switchBuffersIndex].octree.setInputCloud(laserCloudIn.makeShared());
        StoreFrame[switchBuffersIndex].octree.addPointsFromInputCloud();
        std::vector<int> newPointIdxVector;
        StoreFrame[switchBuffersIndex].octree.getPointIndicesFromNewVoxels(newPointIdxVector);

        Eigen::Vector3f direction(1, 1, 0);
        std::vector<PointXYZI, Eigen::aligned_allocator<PointXYZI>> OctreeSearchResult;

        cout << StoreFrame[switchBuffersIndex].octreePre.getIntersectedVoxelCenters(Eigen::Vector3f(0.0f, 0.0f, 0.0f), direction, OctreeSearchResult, 100) << endl;
        ...
        // StoreFrame[switchBuffersIndex].octree.getApproxIntersectedVoxelCentersBySegment();

        StoreFrame[switchBuffersIndex].octreePre.setInputCloud(laserCloudIn.makeShared());
        pcl::copyPointCloud(laserCloudIn, newPointIdxVector, laserCloudIn);

        StoreFrame[switchBuffersIndex].octree.switchBuffers();
        switchBuffersIndex++;
