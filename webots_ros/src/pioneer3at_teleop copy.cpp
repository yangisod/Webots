...
//   updateSpeed();
// }
void VelCallback(const sensor_msgs::PointCloud::ConstPtr &cloud)
{
  pcl::PointCloud<pcl::PointXYZ> laserCloudIn;

  sensor_msgs::PointCloud2 pointcloud2;
  sensor_msgs::convertPointCloudToPointCloud2(*cloud, pointcloud2);
  pcl::fromROSMsg(pointcloud2, laserCloudIn);

  std::vector<int> NaNVec;
  pcl::removeNaNFromPointCloud(laserCloudIn, laserCloudIn, NaNVec);

  Eigen::Matrix4d temp = Eigen::Matrix4d::Identity();
  Eigen::Matrix3d rotation_laser2map(Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitX()));
  temp.block<3, 3>(0, 0) = rotation_laser2map;
  pcl::transformPointCloud(laserCloudIn, laserCloudIn, temp);

  sensor_msgs::PointCloud2 pubcloud;
  pcl::toROSMsg(laserCloudIn, pubcloud);
  pubcloud.header.frame_id = "laser";
  pubcloud.header.stamp = ros::Time().fromSec(cloud->header.stamp.toSec());
  VelPub.publish(pubcloud);
}
void HokCallback(const sensor_msgs::PointCloud::ConstPtr &cloud)
{
  pcl::PointCloud<pcl::PointXYZ> laserCloudIn;
...
void keyboardCallback(const webots_ros::Int32Stamped::ConstPtr &value)
{
  ...
  case 315:
    ...
    speeds[0] = speeds[0] + 1.8;
    speeds[1] = speeds[1] + 1.8;
  ...
  else if (send == 2)
  {
    ...
    ros::ServiceClient set_velocity_client;
    webots_ros::set_float set_velocity_srv;
    set_velocity_client = n->serviceClient<webots_ros::set_float>(std::string("pioneer3at/rotational_motor/set_velocity")); //set_position

    set_velocity_srv.request.value = 3;
    set_velocity_client.call(set_velocity_srv);
  }
}
...
int main(int argc, char **argv)
{
  ...
  // enable lidar
  ros::ServiceClient set_Vel_enable, set_Hok_enable, set_Vel_cloud, set_Hok_cloud, enableKeyboardClient;
  webots_ros::set_int lidar_srv, enableKeyboardSrv;
  webots_ros::set_bool lidar_cloud_srv; //set_bool

  ros::Subscriber sub_lidar_Vel,
      sub_lidar_Hok, sub_keyboard;
  ...
  // set_Vel_cloud = n->serviceClient<webots_ros::set_int>("/pioneer3at/Velodyne_VLP_16/enable_point_cloud");
  // set_Hok_cloud = n->serviceClient<webots_ros::set_int>("/pioneer3at/Hokuyo_UTM_30LX/enable_point_cloud");

  enableKeyboardClient = n->serviceClient<webots_ros::set_int>(controllerName + "/keyboard/enable");

  lidar_srv.request.value = TIME_STEP;
  lidar_cloud_srv.request.value = 1;
  enableKeyboardSrv.request.value = TIME_STEP;
  //&& set_Vel_cloud.call(lidar_cloud_srv) && set_Hok_cloud.call(lidar_cloud_srv)
  if (set_Vel_enable.call(lidar_srv) && set_Hok_enable.call(lidar_srv) && lidar_srv.response.success && enableKeyboardClient.call(enableKeyboardSrv) && enableKeyboardSrv.response.success)
  {
    ROS_INFO("Lidar enabled./keyboard enabled.");
    sub_lidar_Vel = n->subscribe("/pioneer3at/Velodyne_VLP_16/point_cloud", 10, VelCallback);
    sub_lidar_Hok = n->subscribe("/pioneer3at/Hokuyo_UTM_30LX/point_cloud", 10, HokCallback);
